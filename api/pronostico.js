  ...
  try {
    if (req.method === 'GET') {
      ...
      const { rows: predicciones } = await sql`
        SELECT partido_id, goles_local, goles_visitante, penaltis
        FROM predicciones
      ...
      for (const p of predicciones || []) {
        await sql`
          INSERT INTO predicciones
            (jugador_id, fase_id, partido_id, goles_local, goles_visitante, penaltis, actualizado)
          VALUES
            (${jugadorId}, ${faseId}, ${p.partidoId}, ${p.golesLocal}, ${p.golesVisitante}, ${p.penaltis || ''}, now())
